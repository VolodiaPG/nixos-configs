...
sudo launchctl unload /Library/LaunchDaemons/systems.determinate.nix-daemon.plist && sudo launchctl load /Library/LaunchDaemons/systems.determinate.nix-daemon.plist
sudo launchctl kickstart -k system/systems.determinate.nix-daemon

## Determinate

needed to ulimit -n 51200

nix build -v --system x86_64-linux --impure --expr "with import <nixpkgs> { }; runCommand \"check-ulimit\" { } \"echo $(date -u +"%Y-%m-%dT%H:%M:%SZ") > \$out\nulimit -n >> \$out\"" && cat result

## uninstall

First, uninstall nix darwin, then the nix itself

# Removes the delay before the Dock begins to appear

defaults write com.apple.dock autohide-delay -float 0

# Speeds up the slide-in animation to absolute zero (instant pop)

defaults write com.apple.dock autohide-time-modifier -float 0

# Speeds up the drop-down sheets to 0.001 seconds

defaults write NSGlobalDomain NSWindowResizeTime -float 0.001

# Sets an incredibly fast key repeat rate (lower is faster)

defaults write NSGlobalDomain KeyRepeat -int 1

# Minimizes the delay before a key starts repeating

defaults write NSGlobalDomain InitialKeyRepeat -int 10

# Patch transcrypt for warnings spills

curl https://github.com/elasticdog/transcrypt/compare/suppress-openssl-pbkdf2-warnings.patch | patch -p1 .git/crypt/transcrypt

# Bambustudio

Use flatpak
Then use https://github.com/Rdiger-36/StudioBridge with appimage-run to configure the printer in the slicer

# Immich

1. Check what in the dump actually references it (decisive — do this first):
zcat <your-dump>.sql.gz | grep -n 'vectors'
- If you only see CREATE EXTENSION IF NOT EXISTS vectors ... and COMMENT ON EXTENSION vectors ... → it's purely vestigial, proceed.
- If you also see CREATE INDEX ... USING vectors (...) (old pgvecto.rs indexes) → strip those lines too and re-index afterwards (Admin → Jobs → smart search re-index). Current immich doesn't need them.
...
zcat <your-dump>.sql.gz \
  ...
  | gzip > fixed.sql.gz
3. Restore the fixed dump — either upload fixed.sql.gz through the web UI again (now works, since immich is superuser), or do it manually:
...
gunzip -c ~/Documents/fixed.sql.gz | , kubectl --kubeconfig /home/volodia/Documents/k3s.yaml -n immich exec -i database-1 -- psql -d app -v ON_ERROR_STOP=on -q
