...
SYSTEM=$(nix-build --store /mnt/nix "$REPO" -A nixosConfigurations."$TARGET".config.system.build.toplevel --no-out-link)
echo installing system for "$TARGET"...
sudo nixos-install --no-channel-copy --no-root-passwd --system "/mnt/$SYSTEM"
